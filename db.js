...
DB.prototype.help = function() {
    ...
    print("tdb.addUser(username, password)");
    ...
    print("tdb.setProfilingLevel(level) 0=off 1=slow 2=all");
...
DB.prototype.setProfilingLevel = function(level) {
    ...
    if (level < 0 || level > 2) {
        throw {
            dbSetProfilingException: "input level " + level + " is out of range [0..2]"
        };
    }

    if (level) {
        // if already exists does nothing
        this.createCollection("system.profile", {
            capped: true,
            size: 128 * 1024
        });
    }
...
DB.prototype.getCollectionNames = function() {
    ...
    this.getCollection("system.namespaces").find().sort({
        name: 1
    }).forEach(
    function(z) {
        var name = z.name;
...
DB.prototype.getReplicationInfo = function() {
    if ("local" != this)
    return {
        errmsg: "this command only works for database local"
    };

    var result = {};
    var db = this;
    var ol = db.system.namespaces.findOne({
    ...
    var firstc = db.oplog.$main.find().sort({
        $natural: 1
    }).limit(1);
    var lastc = db.oplog.$main.find().sort({
        $natural: -1
    }).limit(1);
    ...
    var last = lastc.next();
    {
        ...
        if (tfirst && tlast) {
            ...
            // low 32 bits are ordinal #s within a second
